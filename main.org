...
(defun dy-hud-tidy ()
  ...
  (projectile-with-default-dir (projectile-acquire-root)
    (async-shell-command
     "cd catkin_ws && find src -type d -name .ccls-cache -prune -o -name '*.cpp' -o -name '*.h' -print | xargs clang-tidy -checks='*,-clang-analyzer-optin.cplusplus.VirtualCall,-llvmlibc-callee-namespace,-modernize-use-trailing-return-type,-llvmlibc-implementation-in-namespace,-modernize-use-trailing-return-type,-llvmlibc-restrict-system-libc-headers,-modernize-use-nodiscard'"
     "*hud-tidy*")))
